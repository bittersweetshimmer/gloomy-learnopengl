...
int main(int argc, char *argv[]) {
  return learnopengl::glfw_context(SCR_WIDTH, SCR_HEIGHT, "gloomy", [&](GLFWwindow &window) {
    ...
    const auto lighting_shader = gloomy::make_ready<gloomy::Program>(
      gloomy::make_ready<gloomy::VertexShader>(learnopengl::load_vertex_shader("assets/shaders/basic_lighting/VertexShader.hlsl").value()),
      gloomy::make_ready<gloomy::FragmentShader>(learnopengl::load_fragment_shader("assets/shaders/basic_lighting/FragmentShader.hlsl").value())
    );
    ...
    learnopengl::loop(window, [&](double time) {
      ...
      gloomy::use([&] {
        ...
        lighting_shader.uniform("u_light_position").fvec<3>(glm::value_ptr(light_position));
